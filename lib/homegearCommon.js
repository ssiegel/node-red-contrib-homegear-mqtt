...
function updatePayload(device, payload, path, values) {
	...
	for(var i in device.read) {
		if(device.read.hasOwnProperty(i)) {
			...
			var key = np[np.length - 1];

			if(param.channel == channel && values.hasOwnProperty(key)) {
				...
				if(np.length == 2) {
					if (!payload.hasOwnProperty(np[0].toLowerCase())) {
						payload[np[0].toLowerCase()] = {};
					}
